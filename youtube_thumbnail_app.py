...
class ThumbnailIdea(BaseModel):
    ...
    headline: str = Field(description="Short overlay text, 2-5 words")  # headline is critical for CTR, should be punchy and clear
    hook: str = Field(description="Why this thumbnail is clickable")  # hook explains the click appeal in a concise way, guiding the visual direction
    visual: str = Field(description="Main visual scene or composition")  # visual describes the dominant imagery or composition
    style: str = Field(description="Art direction and color mood")  # style describes the overall artistic direction and color palette
    prompt: str = Field(description="Detailed image prompt for the thumbnail")  # prompt provides a detailed description for image generation
...
def main():  # sourcery skip: extract-method, use-named-expression
    ...
    if generate:
        ...
        # Loop through the generated ideas and display their components in an organized way, using expanders for better UX
        for item in result["images"]:
            # Create an expander for each thumbnail concept, showing its index and headline in the title for easy identification
            with st.expander(f"#{item['index']} — {item['headline']}", expanded=False):
                # Display the key components of the thumbnail concept in a clear format, using markdown for emphasis and code blocks for the prompt
